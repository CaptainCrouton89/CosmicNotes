...
DECLARE
  ...
  note_record record;
  note_ids bigint[];
  category_enum note_category;
  zone_enum note_zone;
BEGIN
  -- Convert text parameters to enum types if provided
  IF filter_category IS NOT NULL THEN
    category_enum := filter_category::note_category;
  END IF;
  
  IF filter_zone IS NOT NULL THEN
    zone_enum := filter_zone::note_zone;
  END IF;
  ...
  FROM (
    ...
    AND (filter_category IS NULL OR category = category_enum)
    AND (filter_zone IS NULL OR zone = zone_enum)
    ORDER BY score DESC
    LIMIT match_count
  ) AS matched_notes;
